...
export const leaveRoomHandler = async (socket, payload) => {
  try {
    ...
    if (!user) {
      console.error(`존재하지 않는 유저입니다.`);
      throw new CustomError(ErrorCodes.UNKNOWN_ERROR);
    }
    ...
    if (!leaveRoomKey) {
      ...
      throw new CustomError(ErrorCodes.LEAVE_ROOM_FAILED, `사용자가 참여하고 있는 방이 없습니다`);
    }
